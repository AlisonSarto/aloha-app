...
<div class="px-4 pt-8 pb-6">
    <div class="mx-auto w-full max-w-lg">
        ...
        <h1 class="mb-6 text-xl font-bold text-gray-900">Minhas Lojas</h1>

        <div class="space-y-3">
            @foreach($stores as $store)
            <div class="flex items-center justify-between rounded-xl bg-white p-4 shadow-sm ring-1 {{ session('store_id') == $store->id ? 'ring-2 ring-green-500' : 'ring-black/5' }}">
                <div class="min-w-0 flex-1">
                    <p class="truncate font-semibold text-gray-900">{{ $store->name }}</p>
                    @if($store->address_city)
                    <p class="truncate text-sm text-gray-500">{{ $store->address_city }}, {{ $store->address_state }}</p>
                    @endif
                </div>

                @if(session('store_id') == $store->id)
                    <span class="ml-3 flex-shrink-0 inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-700">
