...
<p align="center">
  Plataforma B2B para pedidos de picolés e sorvetes no atacado
</p>
...
O **Aloha App** é uma plataforma B2B de gerenciamento e pedidos de picolés e sorvetes no atacado. O sistema conecta **clientes** (estabelecimentos que revendem os produtos), **administradores** e **vendedores**, com integração nativa à API do **GestaoClick** para sincronização de pedidos, produtos e estoque.

A versão 2.0 está em desenvolvimento ativo e traz uma experiência mais completa de gestão de pedidos com cálculo de preço por faixas de quantidade, múltiplas formas de pagamento e controle de entregas.
...
- Node.js **18+** e NPM
- MySQL **8.0+**
- Credenciais da API **GestaoClick**
...
### Desenvolvimento (todos os serviços em paralelo)
...
- ⚡ Vite com HMR (`npm run dev`)
- 🔄 Processador de filas (`php artisan queue:listen`)
...
npm run dev            # Vite com hot module replacement
npm run build          # Build para produção
php artisan queue:listen  # Processar filas em background
```
...
> Os testes usam banco de dados **SQLite em memória**, configurado automaticamente via `phpunit.xml`. Nenhuma configuração adicional é necessária.
...
# Exibir QR Code de acesso ao app
php artisan qr:show
